...
int button2State = 0;
int button3State = 0;
volatile bool finDeCarreraActivado = false;  // Volátil para las interrupciones
...
// Función de interrupción para el fin de carrera
void IRAM_ATTR endOfRaceInterrupt() {
  finDeCarreraActivado = true;  // Establece la bandera para detener el programa
}
...
void setup() {
  ...
  // Configurar la interrupción en el flanco de bajada para el fin de carrera
  attachInterrupt(digitalPinToInterrupt(endOfRace), endOfRaceInterrupt, FALLING);
...
void loop() {
  ...
  // Si el fin de carrera fue activado, detenemos los motores
  if (finDeCarreraActivado) {
    detenerMotores();
    finDeCarreraActivado = false;  // Reiniciar la bandera
    return;
  }
  ...
  if (cortando == 1) {
    ...
    // Si el motor cuchilla está activo, mueve el motor cuchilla y apaga el motor pan
    else {
